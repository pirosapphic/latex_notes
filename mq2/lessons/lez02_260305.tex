...
L'insieme degli	autovalori propri è detto \textbf{spettro discreto di A}: \textbf{Spd}(A).

L'insieme degli autovalori generalizzati è detto \textbf{spettro continuo di A}: \textbf{Spc}(A).
...
\begin{itemize}
	...
	corrispondenti agli autovalori $ a_i \in$ \textit{Spd}(A);
...
\lim\limits_{n\to\infty} \int_\text{I}^{}dx \left| \psi_n(x)-\psi(x)\right|^2 = 0$$
...
quello di Sistema Completo di Osservabili Commutanti (SCOC), cioè un insieme di osservabili
$ \{ A_1,...,A_n \} $ tutte commutanti tra di loro e tali che, data un'osservabile $B$,
$[B,A_i] = 0\leftrightarrow B =f(A_i,A_2,...,A_n)$: il massimo
numero di osservabili commutanti $ n $ è il numero di gradi di libertà del sistema quantistico.

Dette $ A_i $ le osservabili del SCOC di un sistema quantistico, $\exists\left\{\ket{\alpha}\right\}$ 
insieme di autostati comuni a tutte le $ A_i$:
...
\noindent\fbox{%
    ...
    (base dell'autospazio degenere) abbiano gli stessi autovalori $ \alpha_i $ per le osservabili 
    $A_i$ e si definisca l'osservabile $B$ indipendente dal SCOC t.c. $ B\ket{v_1} = \ket{v_2}$
    e $B\ket{v_2} = \ket{v_1}$:
...
Definiti $ k_I^2 = \frac{2mE}{\hbar^2} $ e $ k_\text{II}^2 = \frac{2m(E-V_0)}{\hbar^2} $, si ha
che l'equazione differenziale è

$$
\begin{dcases}
    \psi'' + k_I^2\psi = 0 &|x| >a \\
    \psi'' + k_\text{II}^2\psi = 0 & |x|<a
\end{dcases}
$$
...
\psi(x) = \begin{dcases}
    ...
    Ce^{ik_\text{II}x} + De^{-ik_\text{II}x} &x\in[-a,a]\\

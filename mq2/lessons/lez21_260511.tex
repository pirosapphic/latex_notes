...
dove $\ket{l,0,k} = \ket{l,m=0,k=k}$ è un autostato simultaneo di $L_z$ ($m=0$) e
...
$$\ket{l,0,k}=\int_{}^{}dk'\sum_{l'=0}^{\infty}\sum_{m'=-l'}^{l'}\ket{l',m',k'}\bra{l',m',k'}
\ket{l,0,k} =\sum_{l'=0}^{\infty}\bra{l',0,k}\ket{l,0,k}\ket{l',0,k}
:=\sum_{l'=0}^{\infty}c_{kl'}\ket{l',0,k}$$
proiettando tale espressione nello spazio delle coordinate si ha
$$\bra{\mathbf{x}}\ket{l,0,k} = \sum_{l'=0}^{\infty}c_{kl'}\bra{\mathbf{x}}\ket{l',0,k} \rightarrow
...
Come prima, si scompone la funzione d'onda in parte radiale e angolare e, per simmetria del
potenziale, la parte angolare sarà indipendente da $\varphi$:
\begin{equation*}
\psi_k(r,\vartheta) = \sum_{l=0}^{\infty}i^l(2l+1)\varphi_l(kr)P_l(\cos\vartheta)
\end{equation*}

\textbf{Oss.} L'\textit{ansatz} della forma della soluzione è la medesima di quella della particella
libera, con la differenza che $\varphi_l(kr)$ non è semplicemente la $j_l(kr)$, ma è la soluzione
dell'equazione radiale con il potenziale.
...
dove non si è posto $\beta_l=0$ perché per $r\not=0$ essa non dà problemi con la normalizzabilità.
...
\textbf{Oss.} Da questa espressione è evidente che le informazioni sull'interazione saranno
contenute in $\tan(\delta_l)$, cioè in $\delta_l$, in quanto questa è l'unica differenza con la
soluzione libera.
...
\end{gather*}
\textbf{Oss.} Come per la particella libera, anche questa soluzione è la sovrapposizione di
...
\boxed{
f_k(\vartheta)=\frac{1}{2ik}\sum_{l=0}^{\infty}(2l+1)e^{i\delta_l}\sin(\delta_l)P_l(\cos\vartheta)
}\end{gather*}
...
{k^2} \sum_{l,l'=0}^{\infty}[...]P_l(\cos\vartheta)P_{l'}(\cos\vartheta) =
\sum_{l=0}^{\infty}\frac{4\pi}{k^2}(2l+1)\sin^2(\delta_l)$$
\textbf{Oss. 1} La sezione d'urto differenziale presenta dei termini di "interferenza" tra le
...
\textbf{Oss. 2} Il teorema ottico è soddisfatto, in quanto
...
\textbf{Oss. 1} La sezione d'urto differenziale è isotropa perché, nell'approssimazione di
$kA$ piccolo, la particella incidente ha $\lambda$ Compton grande e non "vede" la sfera.

\textbf{Oss. 2} La sezione d'urto totale è 4 volte quella geometrica per la natura ondulatoria

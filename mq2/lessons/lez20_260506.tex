...
\textit{Osservazione} Se il potenziale è attrattivo, questa condizione implica che esso non può
...
\textit{Osservazione} Per $\mu\to0$ e $A=zZe^2$ il potenziale si riduce a quello Coulombiano
e la sezione d'urto si riduce a quella di Rutherford: questo è dovuto al fatto che $A$ è la
\textit{costante di accopiamento} del potenziale e $\mu$ dipende dalla \textit{massa del mediatore},
...
nella forma
$$\psi_k(r,\vartheta,\varphi)=\sum_{l,m}X_{k,l,m}\varphi_{k,l,m}(r,\vartheta,\varphi)$$
dove $\varphi_{k,l,m}(r,\vartheta,\varphi)$ sono le autofunzioni simultanee del SCOC 
...
$|L|=|pb|$, la particella interagisce solo per $|L|<L_max = pA$: passando agli autovalori di $L^2$,
...
L'equazione di Schrödinger della particella libera con energia $E=\frac{k^2\hbar^2}{2m}$ è
...
segua \textit{l'equazione radiale ridotta}
...
$$
dove $J^l(z)$ è detta \textit{funzioone di Bessel di I specie}. È chiaro che per $l\in\mathbb{Z}$
le due soluzioni non sono indipendenti, cioè $J^l\propto J^{-l}$: in questo caso si usa un'altra
soluzione dell'ODE, la \textit{funzione di Neumann} $Y_l(z)$.
...
&\text{Funzione di Neumann sferica} \rightarrow \eta_l(z) =\sqrt{\frac{\pi}{2z}}Y_{l+\frac{1}{2}}(z)=
...
\end{dcases}
\end{align*}
Le funzioni di Bessel sferiche sono esprimibili come derivate di funzioni elementari nel modo
seguente:
\begin{align*}
\begin{dcases}
j_0(z) = \frac{\sin z}{z}\\
...
$$\varphi^0_{kl}(\rho) = Aj_l(\rho)+B\eta_(\rho)$$
...
\textit{Osservazione} Questo andamento asintotico evidenzia che a $r\to\infty$ la funzione d'onda sia
...
La soluzione del problema 3D è dunque
$$\varphi^0_{kl}(r,\vartheta,\varphi) = \sqrt{\frac{2k^2}{\pi}}j_l(kr)Y^{lm}(\vartheta,\varphi)$$
\textit{Osservazione} Si consideri la densità di probablilità radiale associata a $\varphi^0_{kl}$:

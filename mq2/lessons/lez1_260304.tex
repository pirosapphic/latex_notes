...
\begin{enumerate}
	    ...
	    \item \textbf{Noto lo stato a $ t=t_0 $, come si può determinare lo stato del sistema
		a $ t > t_0 $?}
	\end{enumerate}
    }%
}%
\subsection{Costruzione della meccanica classica}
...
\noindent\fbox{
    \parbox{\textwidth}{
	\textbf{Lo stato di un sistema fisico a $ t=t_0 $ è rappresentato da un raggio
...
\noindent\fbox{%
	...
	{\large \textbf{Teorema:}} continuità del prodotto scalare
...
i prodotti scalari con $(a,b):=\bra{a}\ket{b}$ ("bra-ket").
...
\sum_{i}^{\infty}\ket{e_i}\bra{e_i} = 1
$$
...
\textbf{Def.} Lo spazio delle distribuzioni temperate è $ S':=\left\{ T:S\to\mathbb{C}/T\text{ è lineare e continua} \right\} $

Il numero $ \bra{T}\ket{g} $ denota l'azione di $ \bra{T}\in S' $ su $ \ket{g}\in S $.
...
\textit{regolare}) definita da $\bra{T}\ket{g} := \int_{-\infty}^{\infty}dxT^*(x)g(x) $
\footnote{Questo processo ricorda l'identificazione degli elementi dello spazio duale}.
...
$ \int_{-\infty}^{\infty}dx\delta f(x) = f(0) $ è un abuso di notazione, perché non 
...
\noindent\fbox{%
    \parbox{\textwidth}{%
    \textbf{Ogni quanità misurabile del sistema fisico è associata a un'osservabile,

...
tale asse, come illustrato nella seguente figura \ref{fig:scattering}.
\begin{figure}[H]
    \centering
...
effetti di interferenza tra particelle che colpiscono bersagli diversi ($\lambda_\text{DeB}<<\text{distanza
tra bersagli}$).
...
sistema e di ridurre l'interazione tra due corpi al moto di uno solo di massa $\mu$ in un potenziale diverso.
...
$$-\frac{\hbar^2}{2m}\laplacian\psi(\mathbf{x})+V(\mathbf{x})\psi(\mathbf{x})=E\psi(\mathbf{x})
\rightarrow\left(\laplacian + k^2\right)\psi(\mathbf{x})=U(\mathbf{x})\psi(\mathbf{x})$$
dove si è ridefinita $\mu:=m$ e si sono definiti
...
d'onda $\mathbf{k}=\frac{1}{\hbar}\mathbf{p}$ e che per $t\to\infty$ tornino a essere onde piane

...
\begin{figure}[H]
    \centering
    \includegraphics[width = \linewidth]{./figures/even_well.png}
    \caption{\small{Soluzioni per le autofunzioni pari}}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width = \linewidth]{./figures/odd_well.png}
...
$$
 \psi(x) =
    \begin{dcases}
	Ae^{\beta_I x} & x< -a \\
	Be^{\beta_\text{II} x}+De^{-\beta_\text{II} x} & -a<x<a \\
	Ge^{-\beta_I x} & x>a
    \end{dcases}
$$
...
\begin{itemize}
	...
	Si ha inoltre che $\forall \varepsilon>0, \exists V\subset\mathbb{R}^3$ t.c.
	$\int_{V}^{}d^3x|\psi|^2 = 1-\varepsilon$, come conseguenza della

...
\newpage
L'altra nozione utile è quella di \textit{limite in media quadratica}
...
\textbf{Esempio 2} L'operatore in 1D $ K $ 
...
Detti $ A_i $ gli operatori del SCOC di un sistema quantistico, per un teorema dell'algebra
lineare $ \exists \left\{ \ket{\alpha} \right\}$ set di autostati comuni a tutti gli $ A_i$:
...
\noindent\fbox{%
    ...
    \textbf{Dimostrazione} Si assuma, per assurdo, che $ \ket{v_1}, \ket{v_2} $ (base dell'
    autospazio degenere) abbiano gli stessi autovalori $ \alpha_i $ per gli operatori $A_i$
    e si definisca l'operatore $B$ indipendente dal SCOC t.c. $ B\ket{v_1} = \ket{v_2}$
    e $B\ket{v_2} = \ket{v_1}$:
...
$\sumint d\alpha_1...d\alpha_n \ket{\alpha_1,...,\alpha_n}\bra{\alpha_1,...,\alpha_n} = 1$

...
$[S_i,S_j] = i\hbar\varepsilon_{ijk}S_k$; dalla teoria del momento angolare si trova che, scelto il SCOC
...
\textbf{Def.} Uno stato misto è una \textbf{sovrapposizione classica} di stati quantistici puri
$\ket{\psi_k}$ tale che la probabilità classica di trovare uno degli stati $\ket{\psi_k}$ sia $\omega_k$,
con
$$
...
\textbf{Oss.} La matrice densità di uno stato puro è il caso banale $\rho_\psi = \ket{\psi}\bra{\psi}$:
...
\begin{enumerate}
    ...
    \item Autovettori e autovalori: se gli stati $\left\{\ket{\psi_k}\right\}$ sono ortonormali,
	$\rho\ket{\psi_l}=\sum_{k}^{}\omega_k\bra{\psi_k}\ket{\psi_l}\ket{\psi_k} = \omega_l\ket{\psi_l}$,
	che è ovvio, in quanto $\rho$ è diagonale nella base $\left\{\ket{\psi_k}\right\}$;
...
$$\ket{\psi} = \sum_{i}^{}c_i\ket{a_i} \text{ e } \rho = \sum_{i}^{}\omega_i\ket{a_i}\bra{a_i}
...
\sum_{j,k}^{}\bra{a_k}\ket{a_i}\bra{a_i}\ket{a_j}\bra{a_j}\ket{a_k}\omega_j=\omega_i
$$
...
non comprende le fasi dei coefficienti di probabilità $\arg(c_i)$, mentre lo stato puro $\ket{\psi}$ sì;
nella probabilità associata a $\rho$ c'è solo il prodotto tra la probabilità classica e quella quantistica, 
in quella associata a $\ket{\psi}$ c'è anche un termine dato dall'interferenza tra le fasi dei coefficienti
$c_i$.

Un altro modo per vedere questa differenza è considerare la matrice densità associata a $\ket{\psi}$,

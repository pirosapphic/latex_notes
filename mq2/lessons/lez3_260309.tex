...
incognite: se il sistema non è singolare ($\det M_{coeff} \neq 0$), l'unica soluzione è $\psi(x)\equiv 0$,
...
\item $\mathbf{V(x)=-\delta(x)}$
    ...
    serve che l'area sia costante: definisco $V_0 =\frac{-A}{2a}$ e $\lim\limits_{a\to0}V(x) = -A\delta(x)$.
...
\begin{itemize}
	...
	a $\laplacian\psi - \alpha^2\psi \sim 0$, che ha come soluzione proprio
	$\psi(\vec{x}) \underset{|\vec{x}|\to\infty}\sim e^{-\beta |\vec{x}|}$;
	essenod $\psi$ quadrato-sommabile, $E\in Spd(H)$

...
\textbf{Esempio, stato misto}\\
Si consideri la stessa situazione dell'esempio precedente, dato però lo
stato misto
...
\begin{align*}
    ...
    &=\sum_{\alpha}\omega_\alpha\sum_{m,n} c_{im}^{(\alpha)}c_{in}^{(\alpha)*}\ket{a_i,b_m}\bra{a_i,b_n}:=
    \rho'
\end{align*}
...
\left(\sum_{k=0}^{n-1}{n-1\choose k}H^{n-k-1}[H,F]H^{k}\right)\right]=0
\end{gather*}
se e solo se $[H,F] = 0$.

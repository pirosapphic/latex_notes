...
\int d^fq\bra{q_b}U(t_b,t_a)\ket{q}\psi(q,t_a)
\label{eq:K_def}
\end{equation}
...
per la descrizione della propagazione delle onde d'alambertiane nel tempo.
...
\int dq^f_1\bra{q_b}U(t_b,t_1)\ket{q_1}\bra{q_1}U(t_1,t_a)\ket{q_a}
$$
Iterando per $N$ istanti di tempo $t_i$ tali che $t_a=t_0<t_1<...<t_N=t_b$ si ottiene che
\begin{equation}
    K(b,a)=\int \prod_{i=1}^{N-1} d^fq_i \prod_{j=0}^{N-1}\bra{q_{j+1}}U(t_{j+1},t_j)\ket{q_j}
    \label{eq:K_iter}
\end{equation}
che significa che il propagatore dipende da \textit{tutti i possibili cammini tra
...
$$
i\hbar\pdv{K}{t}=-\frac{\hbar^2}{2m}\pdv[2]{K}{x}+VK \to
\begin{dcases}
i\hbar\pdv[]{K}{t}=-\pdv[]{S}{t}e^{\frac{i}{\hbar}S}\\
-\frac{\hbar^2}{2m}\pdv[2]{K}{x}+VK=e^{\frac{i}{\hbar}S}\left[-\frac{\hbar^2}{2m}\left(-\frac{1}{\hbar^2}
\left(\pdv[]{S}{x}\right)^2 + \frac{i}{\hbar}\pdv[2]{S}{x}\right)+V\right]
\end{dcases}
$$
e si ottiene per $S$
...
sulla curva di moto classica che congiunge $q$ e $q'$.
...
cioè un integrale che somma i valori della funzione d'onda iniziale su tutti i $q$ dello spazio, pesati con
